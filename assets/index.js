...
const control = [];												//création d'un objet array pour envoyer les _id dans le localStorage → contrôle des données transmises dans l'URL et message d'erreur 404 si non conformes
...
function catalog(data) {										//fonction qui remplit le catalog avec les données récupérée
	...
	for(const i in data) {	//version avec indice pour gestion des animations 	//pour chaque cellule du tableau
		...
		product.setAttribute('class', 'section section-horizontal-center anim anim-' + i);	//style de la div qui affiche le produit
		...
		`;
		control.push('teddyId=' + data[i]._id);
	}
...
/*	
	for(const x of data) {									//autre code pour function catalog pour chaque cellule du tableau
